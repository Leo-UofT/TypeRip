var opentype = (() => {
    ...
    var name_default = { parse: parseNameTable, make: makeNameTable };
    ...
    var cpal_default = { parse: parseCpalTable, make: makeCpalTable };
    ...
    function fontToSfntTable(font) {
      ...
      const fvarTable = font.tables.fvar ? fvar_default.make(font.tables.fvar, font.names) : void 0;
      const languageTags = [];
      ...
      const postTable = post_default.make(font.tables.post);
    ...
    function thaiGlyphComposition(range) {
      const script = "thai";
      let tokens = this.tokenizer.getRangeTokens(range);
      let contextParams = getContextParams3(tokens, 0);
      for (let index = 0; index < contextParams.context.length; index++) {
        contextParams.setCurrentIndex(index);
        let substitutions = this.query.lookupFeature({
          tag: "ccmp",
        ...
        if (substitutions.length) {
          ...
          contextParams = getContextParams3(tokens, index);
        }
      }
    }
    ...
    function parseBuffer(buffer, opt = {}) {
      ...
      if (gvarTableEntry) {
        ...
        font.tables.gvar = gvar_default.parse(gvarTable.data, gvarTable.offset, font.names);
      }
      if (avarTableEntry) {
        if (!fvarTableEntry) {
